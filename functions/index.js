...
exports.addEmail = functions.https.onRequest((request, response) => {
	...
	if (validator.validate(email)) {
		db.collection('email_list').where('email', '==', email).get().then((querySnapshot) => {
			...
			if (testBool) {
				db.collection('email_list').doc(docId).update({ email: email, updated_on: new Date(new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })) }).then(() => {
				...
				db.collection('email_list').add({ email: email, created_on: new Date(new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })) }).then((doc) => {
					docId = doc.id;
...
exports.getEmails = functions.https.onRequest((req, res) => {
	...
	db.collection('email_list').orderBy('created_on', 'desc').get().then((querySnapshot) => {
		...
		res.setHeader('Content-disposition', 'attachment; filename=exporentals_emails_' + new Date(new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }))
			+ '.csv');
